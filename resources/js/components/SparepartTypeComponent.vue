<template>
  <div>
    <div class="text-xs-center">
        <h1 style="font-size:15pt; padding-top:30px">TABEL TIPE SPAREPART</h1>    
    </div>
    <!-- ini pop up tambah-->
      <v-dialog v-model="dialog" persistent max-width="600px">
       <template v-slot:activator="{ on }">
        ...
        <v-card>
          <v-card-title>
            <span class="headline">Tambah Tipe Sparepart</span>
          </v-card-title>
          <VForm>
          <v-card-text>
            <v-layout>
      ...
      <v-dialog v-model="dialog2" persistent max-width="600px">
        <v-card>
          <v-card-title>
            <span class="headline">Edit Tipe Sparepart</span>
          </v-card-title>
          <VForm>
          <v-card-text>
            <v-layout>
      ...
      >
        ...
        <td>
          <v-icon
            small
            class="mr-2"
            @click="editTipe(props.item.id)"
          >
            edit
          </v-icon>
         <v-icon
            small
            @click="deleteTipe(props.item.id)"
          >
            delete
          </v-icon>
        </td>
...
import { mapGetters, mapState, mapActions } from 'vuex'
  ...
  export default {
    ...
    ...mapState({
      ...
      error: state => state.Spareparttype.error,
      items: state => state.Spareparttype.spareparttypes
    }),
    ...mapGetters({
      form: 'Spareparttype/spareparttype',
    })
  },
    ...
    methods: {
      ...
      ...mapActions({
        ...
        add: 'Spareparttype/store',
        edit: 'Spareparttype/update',
        find: 'Spareparttype/edit',
        delete: 'Spareparttype/delete'
      }),

      async submitHandler()
      {
          const payload = {
            nama: this.form.type_name,
          }
      ...
      async deleteHandler () {
        console.log(this.id_sparepart_type);
        await this.delete(this.id_sparepart_type)
        this.fetch()
        this.dialog3=false
      },
